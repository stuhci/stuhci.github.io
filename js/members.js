...
document.addEventListener('DOMContentLoaded', function() {
  ...
  async function renderMembersPage() {
    ...
    facultySection = renderTypeSession('Faculty', membersByType['Faculty'])
    membersContainer.appendChild(facultySection)

    gradStuSection = renderTypeSession('Graduate Students', membersByType['Graduate Students'])
    membersContainer.appendChild(gradStuSection)

    underGradSection = renderTypeSession('Interns', membersByType['Intern'])
    membersContainer.appendChild(underGradSection)

    alumniSection = renderTypeSession('Alumni', membersByType['Alumni'])
    membersContainer.appendChild(alumniSection)

    // enhancePublicationAbstracts();
  }

  function renderTypeSession(typeName, typeList){
     const teamSection = document.createElement('div');
      teamSection.className = 'team-section';
      ...
      const teamTitle = document.createElement('h2');
      teamTitle.className = 'section-subtitle';
      teamTitle.textContent = typeName;
      teamSection.appendChild(teamTitle);
